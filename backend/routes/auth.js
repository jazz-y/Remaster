...
const { mongoose, db, User, Flashcard, FlashcardSet } = require('../middleware/dbConnect.js');
...
authRouter.post('/login',
  passport.authenticate('local'), async (req, res) => {
    try {
      ...
      verify = true;
      if (!verify) {
        return res.status(401).json({ message: "Authentication failed: " + info, isAuthenticated: false });
      }
      res.json({ message: "User authenticated!", username: req.user.username, isAuthenticated: true });
    } catch (err) {
      console.log(err);
...
authRouter.get('/index', async (req, res) => {
  try {
    // console.log(req.user);
    // const verify = req.isAuthenticated();
    const verify = true;
    ...
    // Use await
    // console.log(req.body + "printing req.user");
    // const curUser = await User.findOne({ username: req.user });
...
authRouter.get('/set', async (req, res, cb) => {
  ...
  try {
    const verify = true; // for now
    const returnSet = await FlashcardSet.findById(process.env.FLASHCARD_SET_ID).exec(); //existing set - for now
    returnCards = [];
